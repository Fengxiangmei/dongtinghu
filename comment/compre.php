    ...
    chart = new Highcharts.Chart({ 
        ...
        series: [{  //数据列 
           ...
           //data: [<?php echo $As.','.$Hg.','.$Cd.','.$Cr.','.$CODMn.','.$COD.','.$BOD5.','.$NH3_N.','.$P.','.$Petroleum.','.$Anionicsurfactant.','.$Sulfid.','.$Coliforms;?>] 
			data:[<?php for($i=0;$i<count($data);$i++){  echo sprintf("%.4f", $data[$i]).",";}?>],
			 dataLabels: {
                enabled: true,
                rotation: 0,
                color: '#000',
...
<div id="divise"></div>
<div id="content"><br/><p class="title">污染分子数均值评价</p><br/>
<form action="compre.php" method="post" onSubmit="return Check(this)" name="Form"> 
<p class="title">
时间：
